...
| REQ-HOME-WATER-ALARM-001 | P7.S4 | ✅ | `notification/WaterReminderScheduler.kt` (IN-PLACE rewrite — Class-Name behalten, Logik = Defizit-Tick `checkIntervalMin` default 30 min), `notification/AlarmReceiver.kt::handleWaterFire` (NEW: liest `applyOverrides(profile).waterMl`, `expectedWaterByNow` linear 08–22, conditional notify ≥ `deficitThresholdMl` default 200 ml, immer chained), `notification/WaterReminderPrefs.kt` (NEUE Felder `checkIntervalMin` + `deficitThresholdMl`). Active-Window-Clamp 08–22 → de-facto Silent-22-08. Eskalation 30→15→10→5 min nicht implementiert (Slice 4c.1 möglich). (Slice 4c 2026-05-28) |
