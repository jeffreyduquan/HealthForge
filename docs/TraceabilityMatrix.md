...
| REQ-INGR-003 | P1 / P7.S3 Slice 2 / P7.S3 Hotfix-5 | ✅ | `server/etl/Importers.kt::SighiImporter` re-implementiert als Keyword-Matcher (substring-/akzent-insensitiv), liest `seed/sighi.csv` (kuratiert aus SIGHI-Merkblatt v2021-11-17 PDF). End-to-End ✅ 2026-05-29: ETL `POST /admin/v1/etl/run?source=SIGHI` → `etl_runs.status=SUCCESS`, 477/632 Ingredients erhielten Score 0/1/3 (Verteilung: 284 gut/38 unsicher/155 zu meiden/155 unbekannt). Spot-Check: Salami=3, Tomate=3, Avocado=3, Spinat=3, Camembert=3, Mozzarella=0, Apfel=0, Hähnchenbrust=0, Knoblauch=0, Kabeljau=0. **Hotfix-5 (2026-05-31):** Audit-Snapshot zeigte 150 NULL-Scores. sighi.csv um ~140 Keywords erweitert (Compounds/Region/Sorte): Mortadella/Bergader/Pak Choi/Cantaloupe/Bagel/Pumpernickel/Hefe/Mayonnaise/Pralinen/Sherry/Algen Nori etc. ETL `c71bd613-…` + `2aea5a8a-…`. **Histamin-Coverage final: 636/636 (100 %).** Verteilung 380×0 / 60×1 / 196×3. |
...
| REQ-DATA-TRANSLATE-001 | P7.S2 | ✅ | `server/.../tools/TranslateFdcNames.kt` (NEW Slice 3b 2026-05-28, Standalone-Kotlin-Tool) + Gradle-Task `:translateFdcNames`. DeepL Free API (`api-free.deepl.com/v2/translate`), Batches à 50, atomic-rename nach jedem Batch (interrupt-safe), HTTP-429/503 Expo-Backoff. **Voller Lauf 4m 48s, 8251 Rows + 100 Smoke + 3 Demo = 8354/8354 ✓ Coverage 100%.** Stichprobe-Quality: Alaska Pollock → Alaska-Seelachs, Almond butter → Mandelbutter. DeepL-Quota ~210k/500k Monat verbraucht. Altes `tools/translate_fdc_names.main.kts` gelöscht. Admin-UI `FdcTranslationsPage.tsx` geparkt auf P7.S5. **Hotfix-2 2026-05-31:** Translation-Mismatch-Audit `server/tools/audit_translations.ps1` (35 Trigger/Required-Regeln, scant alle 638 curated Rows) fand 149 Mismatch-Kandidaten, davon **26 hart-falsche DeepL-Halluzinationen** (z. B. fdc=168048 Salmonberries→„Räucherlachs", fdc=169222 Yardlong bean→„Vanille Schote", fdc=171714 Breadfruit→„Gnocchi", fdc=169334 Butterbur→„Ghee", fdc=169886 Chicken meatless→„Hähnchenhack"). Korrigiert via `server/tools/patch_translations.ps1` (fdc_id→neuer name_de Override-Map) direkt im `usda_fdc_curated.csv`. Re-Import: SUCCESS 2.2s, 0 inserted, **632 updated**, 6 skipped (etl_run `fdf60d29-…`). 26/26 Korrekturen in DB verifiziert. Verlorene kanonische DE-Foods → Folge-Slice. |
| REQ-DATA-CURATION-001 | P7.S3 Slice 1 | ✅ | NEW 2026-05-29. `server/.../tools/CurateUsdaSeed.kt` (Standalone-Kotlin-Tool, read-only auf `usda_fdc.csv` + `fdc_top_ids.csv`, schreibt `usda_fdc_curated.csv` + `curation_report.md`) + Gradle-Task `:curateUsdaSeed`. **Pipeline-Resultat:** 8354 Rows → 3769 nach Filtern (-300 wrongDataType, -46 noTranslation, -43 noMacros, -144 tooFewMicros, -4052 blacklisted) → 3719 nach Dedupe → **1500 final (Top-N nach Quality-Score)**. Blacklist deckt NFS, baby/babyfood/junior/toddler/strained, infant formula, fast food, restaurant, candies, snacks, dietary supplement, leavening agents, spices mixed, Komma-Modifier-Ketten ≥ 4 ab. `UsdaFdcImporter.seedResourcePath()` zeigt auf `seed/usda_fdc_curated.csv` mit Fallback `seed/usda_fdc.csv`. Flyway `V14__curated_ingredients_reset.sql` (DESTRUKTIV, Pre-Launch-User-Bestätigung) trunkiert `ingredients`, `recipes`, `recipe_ingredients`, `recipe_reports`, `ingredient_field_pr`, `etl_runs` via `information_schema`-Existenz-Check. Compile-Verify: `:compileKotlin` BUILD SUCCESSFUL 23s. **End-to-End-Boot-Verifikation ✅ 2026-05-29:** Server-Start 17.7s, Flyway V1–V14 applied, ETL `POST /admin/v1/etl/run?source=USDA_FDC` → `etl_runs.status=SUCCESS`, `SELECT count(*) FROM ingredients = 1500` (100 % `source='USDA_FDC'`). **Hotfix Restaurant-Chain-Blacklist ✅ 2026-05-29:** App-Smoketest fand 87 US-Chain-Entries (APPLEBEE'S, BURGER KING, McDONALD'S etc.) — sind `SR Legacy` getaggt, wurden nicht durch `\brestaurant,\s+` gefiltert. Fix: 46-Brand-Whitelist mit `RegexOption.IGNORE_CASE` (McDONALD'S hat Lowercase-`c`). Re-Import: 1500 Rows, **0 Chains** verifiziert. **SUPERSEDED 2026-05-29 durch REQ-DATA-CURATION-002** (App-Smoketest ergab dass Top-1500-Quality-Set zu USA-lastig + zu viele Cocktails/Convenience-Mixes/Near-Duplikate). Tool bleibt für Fallback intakt. |
| REQ-DATA-CURATION-002 | P7.S3 Slice 1 Hotfix | ✅ | NEW 2026-05-29. **Whitelist-driven Curation (Strategy B):** kuratierte deutsche Essentials-Liste `seed/essentials_de.csv` (638 Einträge, 24 Kategorien) + Matcher `tools/CurateByWhitelist.kt` (Gradle-Task `:curateByWhitelist`) der Token-Overlap-Score mit Form-Penalty (juice/peel/powder/dehydrated −25..−50 wenn Query sie nicht erwähnt) und 2-Pass-Strategie (Threshold≥40 first, dann Fallback MIN_SCORE=10) gegen Voll-Seed `usda_fdc.csv` matcht. Output überschreibt `usda_fdc_curated.csv`. **Run-Resultat:** 638→638 Matches (100 %), avg-Score 59. **Re-Import-Verifikation:** ETL-Run `079d2527-...` SUCCESS, 632 Rows in `ingredients` (6 skipped wg invalider USDA-Daten), Sample: Apfel/Banane/Hähnchenbrust/Hüttenkäse/Vollkornbrot/Lachs/Olivenöl/Zwiebel. 0 US-Chains, 0 Cocktail-Pollution. App-Smoketest erfolgreich. |
| REQ-INGR-MICRONUTRIENTS-001 | P7.S1 / P7.S5 4f | ✅ | **Server (P7.S1):** Flyway `V12__nutrients_overhaul.sql` (ALTER ingredients: micronutrients_json JSONB + fdc_id BIGINT UNIQUE + GIN-Index); `IngredientEntity` + `IngredientDto.from()` extracted; BuildUsdaSeed/UsdaFdcImporter befüllen 24 USDA-Nutrient-IDs auf 24 Catalog-Keys. **Client (P7.S5 4f, 2026-05-29):** `data/network/IngredientApi.IngredientDto` erweitert um `fdc_id: Long? = null` + `micronutrients: Map<String, Double> = emptyMap()`. UI-Sichtbarmachung im `presentation/lebensmittel/components/IngredientDetailSheet.kt`. **Audit gegen Produktiv-DB (2026-05-29):** 8326/8354 (99.7 %) ≥ 1 Mikro, 7340 (87.9 %) ≥ 10 Mikros, 5549 (66 %) ≥ 20 Mikros. Top-Coverage Na 99 %, Fe/Ca 98 %, B-Vitamine 88-90 %. Lücken B7 1.2 %, Jod 0.4 % (USDA misst selten). |
| REQ-INGR-DETAIL-SHEET-001 | P7.S5 4f | ✅ | NEW `presentation/lebensmittel/components/IngredientDetailSheet.kt` (`ModalBottomSheet` mit Header + Source-Badge + Makros + Mikronährstoffe gruppiert per `NutrientCatalog.ofCategory` mit %-DGE-Pill + Allergene/FODMAP/Histamin conditional). MOD `presentation/lebensmittel/LebensmittelScreen.kt` (`detailTarget: IngredientDto?`-State, `IngredientRow` neuer `onOpenDetail`-Param: preselect ⇒ `onPick`, sonst ⇒ Sheet-Open). Verifikation `:app:installDebug` BUILD SUCCESSFUL 28s auf Pixel_7_API_35. Sichtbarkeit Histamin-Block + FODMAP-Chips blockiert bis SIGHI-Pipeline / FODMAP-Mapper (separate Slices). |
| REQ-INGR-ALLERGEN-MAPPING-001 | P7.S2 / P7.S3 Hotfix-4 | ✅ | `server/.../etl/usda/AllergenMapper.kt` (Object, 17 Codes = EU-14 + HISTAMINE/TYRAMINE/ALCOHOL). Slice 3a: `NEGATIVE_LIST` (`mustard-seed-oil`/`mustard oil` + `coconut*` + `nutmeg`) per `stripNegatives()` Pre-Match. Test `AllergenMapperTest` 10/10 grün. Wired via `UsdaFdcImporter` Zeile 85: `extractAsStrings("$nameEn $ingredientsEn")`. **End-to-End-Verifikation 2026-05-28 (Slice 3c) auf 8354 produktiven Rows:** Bagels Weizen→GLUTEN, Senfgrün→MUSTARD, Erdnussbutter-Riegel→PEANUT+LACTOSE, Eis mit PB→PEANUT+SOY+LACTOSE, Muskateller-Wein→ALCOHOL (kein NUT), Kokosnussmehl/-öl/-wasser→`[]` (NEGATIVE_LIST greift in Produktion ✓). **Hotfix-4 (2026-05-31):** KEYWORDS-Map erweitert um ~22 Plural-/Compound-Forms (GLUTEN: bread/bagel/noodle/pasta/pita/ciabatta/focaccia/tortellini; LACTOSE: kefir/yogurts/milks/creams/cheeses; SOY: soymilk/soyabean; NUT: hazelnuts/walnuts/pecans/cashews/pistachios/macadamias; CRUSTACEAN: shrimps/prawns/lobsters/crabs; FISH: sardines; MOLLUSC: oysters/clams/scallops/squids/snails). Hafer/Oats bewusst NICHT in GLUTEN (botanisch glutenfrei). Re-Import etl_run=`912e817d-…`, DB-Delta flagged: 188→212 (+24). Spot-Check Mehrkornbagel/Kefir/Pekannüsse/Sojamilch verifiziert. |
| REQ-HOME-NUTRIENT-LIST-001 | P7.S3 / P7.S3.b / P7.S4 4e (Revision) | 🟢 | `presentation/home/HomeScreen.kt` (Layout-Refactor + Card-Wiring), `PinnedNutrientCard.kt` (NEW P7.S3 + P7.S3.b Stufen-Bar + P7.S4 4e Revision: Header/Chevron-only + Expanded-Inline-Picker mit Kategorie-Sections), `NutrientListSection.kt` (NEW), `NutrientRow.kt` (NEW), `PlannedMealRow.kt` (NEW), `HomeViewModel.togglePin/reorderPins/parsePinnedKeys/togglePinsExpanded` (Persistenz `UserProfileEntity.pinnedNutrientsJson`). Default-Pins = `{kcal, protein, carbs, fat, water}`. Min-1-Pin-Invariant. Stift-Edit-Modus + `NutrientPinPickerSheet` in Revision 2026-05-28 entfernt (User-Direktive: 1 Chevron statt 3 Affordances).
...
| REQ-HOME-WATER-ALARM-001 | P7.S4 | ✅ | `notification/WaterReminderScheduler.kt` (IN-PLACE rewrite — Class-Name behalten, Logik = Defizit-Tick `checkIntervalMin` default 30 min + Eskalation 30→15→10→5 min via `currentIntervalMin()` + `prefs.escalationLevel`), `notification/AlarmReceiver.kt::handleWaterFire` (NEW: liest `applyOverrides(profile).waterMl`, `expectedWaterByNow` linear 08–22, conditional notify ≥ `deficitThresholdMl` default 200 ml, Eskalation++ bei Notify / Reset bei No-Defizit, immer chained), `notification/WaterReminderPrefs.kt` (NEUE Felder `checkIntervalMin` + `deficitThresholdMl` + `escalationLevel`). Active-Window-Clamp 08–22 → de-facto Silent-22-08. Snooze-Action im UI nicht impl. (Slice 4c + 4c.1 2026-05-28) |
| REQ-PROFILE-LAYOUT-001 | P7.S4 | ✅ | Slice 4a ✅ (2026-05-28): `presentation/profile/ProfileScreen.kt` (DROP `WASSERZIEL`-Section + `ANGEHEFTETE NÄHRSTOFFE`-Chip-Grid; EXPAND `TAGESZIELE` über `domain.nutrition.NutrientCatalog.all`, 33 Einträge, kategorie-gruppiert). NEW `presentation/profile/components/NutrientGoalRow.kt` (Default read-only + Override-NumberField + Reset-Icon). MOD `ProfileViewModel` (inject `ComputeNutrientTargetsUseCase` für `computedDefaults`, NEW `clearNutrientGoal`/`resetWaterGoalMl`, `setNutrientGoal("water",…)` routet auf `setWaterGoalMl`). DEL `presentation/profile/NutrientCatalog.kt` (P6.S6-Legacy). Slice 4d ✅ (2026-05-28): `NutrientGoalRow.kt` rewritten — Lock-Slider Range 0..2× Default (=0–200 %), `Outlined.Lock`/`LockOpen` ephemeral UI-Toggle, Commit auf Re-Lock, Display `{absoluter Wert} + Prozent` + Default-Hint bei aktivem Override. `ProfileScreen.kt` NEW Section "ALLERGIEN & INTOLERANZEN" mit `FlowRow` + `FilterChip` (14 EU-Allergene + 5 FODMAP-Typen, inline-toggle). `ProfileViewModel` NEW `setAllergies`/`setIntolerances` (delegate `replaceAllergies`/`replaceIntolerances`). Kein Room-Bump. Verifikation: `:app:compileDebugKotlin` BUILD SUCCESSFUL 21s, Visual-Smoke Pixel_7_API_35 ✓. |
| REQ-PLAN-WATER-GOAL-001 | P7.S4 | ❌ DEFERRED | Slice 4b kurzzeitig implementiert (Commit `2c44b3b`), nach User-Feedback "Ziele sind nur im Profil anpassbar" revertiert (`61c6389` 2026-05-28). Mental Model = Tagesziele sind global pro User. Profil-Lock-Slider (REQ-PROFILE-LAYOUT-001 / Slice 4d) bietet stattdessen volle 0–200 %-Bandbreite. `MealPlanSlotEntity.waterGoalMl: Int?`-Spalte bleibt in DB (P7.S1 Schema v8, unused, kein Migration-Aufwand). |
